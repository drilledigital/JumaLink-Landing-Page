 ...
 * Only redirects phones/tablets to a store; desktop does nothing here (see /app-store page UI).
 * Logic must match {@link getStoreRedirectKind}.
 * @param {{ playStoreUrl: string; appStoreUrl: string }} urls
...
export function buildAppStoreImmediateRedirectScript(urls) {
  ...
  return `(function(){try{var ua=navigator.userAgent||"";var t=typeof navigator.maxTouchPoints==="number"?navigator.maxTouchPoints:0;var p=${p},a=${a},k;if(/Macintosh/i.test(ua)&&t>1)k="s";else if(/iPhone|iPod|iPad/i.test(ua))k="s";else if(/Android/i.test(ua))k="p";else return;if(k==="p")location.replace(p);else location.replace(a);}catch(e){}})();`;
}
